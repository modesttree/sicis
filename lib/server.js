...
var express = require('express'),
    ...
    repoWatcher = require('./repoWatcher');
...
var _addIoReceivers = function() {
    ...
    _io.on('connection', function(socket){
        socket.on('forceBuild', function() {
            log.info("Received request to force trigger build");
            repoWatcher.forceTriggerBuild();
        });

        socket.on('forcePoll', function() {
            log.info("Received request to force poll repo");
            repoWatcher.forcePollRepo();
        });
    });
};
...
var _addHttpHandlers = function(onFinished) {
    ...
    _app.get('/status', function(req, res) {
        res.render('status', {
            ...
            buildLog: _buildLog,
            gitRepoUrl: repoWatcher.repoUrl,
