...
var _addHttpHandlers = function(onFinished) {
    ...
    if (_currentLogPath) {
        _app.get('/detailedCurrentLog.txt', function(req, res) {
            res.sendFile(_path.join(_options.repoPath, _currentLogPath), {headers: {'Content-Type': 'application/force-download'}});
        });
    }

    if (_previousLogPath) {
        _app.get('/detailedPreviousLog.txt', function(req, res) {
            res.sendFile(_path.join(_options.repoPath, _previousLogPath), {headers: {'Content-Type': 'application/force-download'}});
        });
    }
