...
var _execBuildCommand = function(command, callback) {
    ...
    child.on('error', function(err) {
        _appendToBuildLog("Error while running '{0}': {1}\n".format(command, err.message), LogTypes.Error);
    });
};
...
var _runBuild = function() {
    ...
    _httpServer.setStatus('<div class="running">Build running</div>')
...
var _onBuildComplete = function(succeeded) {
    ...
    if (succeeded) {
        ...
        _appendToBuildLog('Build completed with errors\n', LogTypes.Error);
    }
...
var _pollRepoAndInvokeBuilds = function() {
    ...
    _httpServer.setStatus('<div class="polling">Polling Git</div>')
...
var _delayThenPollAgain = function() {
    ...
    _assert(!_pollingOrBuildInProgress);
    _httpServer.setStatus('<div class="idle">Idle</div>')
