...
var _maxConcurrentBuilds = null;
var _pollInterval = null;
var _sicisRootPath;
var _inProgressFilePrefix = "job_inprogress_";
...
var _execCommand = function(command, callback, errorCallback) {
...
var _fetchRemote = function(callback, errorCallback) {
    _execCommand('git fetch ' + _remoteName, callback, errorCallback);
};
...
var _checkRepoForChanges = function(callback) {
    ...
    _execCommand('git reset --hard {0}/{1}'.format(_remoteName, _branchName), function(succeeded) {
        _assert(succeeded);
        ...
        _fetchRemote(function() {
            ...
            function(err) {
                _log.info("Error occurred while executing 'git fetch'!  Skipping attempt.");
                callback(false);
            });
    });
};
...
var _inprogressFilePath;
var _hasNotifiedWaitingForLessBuilds;
var _buildStartTime;
...
var _waitToStartBuild = function() {
    ...
    if (_maxConcurrentBuilds) {
        _hasNotifiedWaitingForLessBuilds = false;
...
var _waitToStartBuildInternal = function() {
    _countCurrentBuilds(function(numBuilds) {
        if (numBuilds < _maxConcurrentBuilds) {
            // wx so it fails on existing files
            _fs.open(_inprogressFilePath, "wx", function (err, fd) {
                _assert(!err)
            ...
            if (!_hasNotifiedWaitingForLessBuilds) {
                _hasNotifiedWaitingForLessBuilds = true;
...
var _countCurrentBuilds = function(callback) {
    _fs.readdir(_sicisRootPath, function(err, items) {
        var count = 0;
        for (var i=0; i<items.length; i++) {
            if (items[i].startsWith(_inProgressFilePrefix)) {
                count += 1;
            }
        }
...
var _onBuildComplete = function(succeeded) {
    ...
    if (_maxConcurrentBuilds) {
        _removeInProgressFileIfExists();
    }

    _delayThenPollAgain();
};

var _removeInProgressFileIfExists = function() {
    if (_fs.existsSync(_inprogressFilePath)) {
        _fs.unlinkSync(_inprogressFilePath, function(error) {
        });
    }
}
...
module.exports.start = function(buildCommand, autoBuild, maxConcurrentBuilds, pollInterval, sicisRootPath, onStarted) {
    ...
    _inprogressFilePath = _path.join(sicisRootPath, _inProgressFilePrefix + Math.trunc(9999999999 * Math.random()) + '.lock');
