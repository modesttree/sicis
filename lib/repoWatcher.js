...
var _getBuildTimeDeltaStr = function() {
    var timeDiff = _timeDiff(_buildStartTime, Date.now(), 'Hm');

    if (timeDiff.hours == 0) {
        return '{0} minutes'.format(timeDiff.minutes)
    }
    else {
        return '{0} hours, {1} minutes'.format(timeDiff.hours, timeDiff.minutes)
    }
};

var _getBuildTimeDeltaWithSecondsStr = function() {
    var timeDiff = _timeDiff(_buildStartTime, Date.now());
    return '{0} hours, {1} minutes, {2} seconds'.format(timeDiff.hours, timeDiff.minutes, timeDiff.seconds)
};
...
var _runBuild = function() {
    ...
    _httpServer.setElapsedTime('Elapsed: ' + _getBuildTimeDeltaStr())
...
var _onBuildComplete = function(succeeded) {
    ...
    _httpServer.setElapsedTime('Total time: ' + _getBuildTimeDeltaWithSecondsStr())
...
module.exports.start = function(buildCommand, autoBuild, singleBuildOnly, sicisRootPath, onStarted) {
