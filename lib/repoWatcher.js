...
var _execBuildCommand = function(command, callback) {
    ...
    _appendToBuildLog('Executing: ' + command);
    ...
    child.stdout.on('data', function(output) {
        _appendToBuildLog(output.trim());
    });

    child.stderr.setEncoding('utf8');
    child.stderr.on('data', function(output) {
        _appendToBuildLog(output.trim(), LogTypes.Error);
    });
    ...
    child.on('error', function(err) {
        _appendToBuildLog("Error while running '{0}': {1}".format(command, err.message), LogTypes.Error);
    });
};
...
var _updateThenRunBuild = function() {
    ...
    _execBuildCommand('git reset --hard', function(succeeded) {
        if (succeeded) {
...
var _onBuildComplete = function(succeeded) {
    ...
    if (succeeded) {
        _appendToBuildLog('Build completed successfully', LogTypes.Good);
    }
    else {
        _appendToBuildLog('Build completed with errors', LogTypes.Error);
    }
